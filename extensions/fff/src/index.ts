/**
 * fff: lean FFF find/grep for our pi harness.
 *
 * FFF engine with a stock-pi-shaped tool surface:
 * - default mode: override (tools named find/grep)
 * - no always-on promptGuidelines (stock find/grep also have none)
 * - stock-style truncation (truncateHead @ 50KB) + Grep/Find details
 * - no multi_grep (removed; plain grep is enough)
 *
 * Upstream: https://github.com/dmtrKovalenko/fff (packages/pi-fff)
 */
...
const DEFAULT_GREP_LIMIT = 12;
const DEFAULT_FIND_LIMIT = 20;
const GREP_MAX_LINE_LENGTH = 500;
const MENTION_MAX_RESULTS = 20;
const THRASH_WINDOW_MS = 120_000;
...
// ---------------------------------------------------------------------------
// Thrash / duplicate-query guard (process-local)
// ---------------------------------------------------------------------------
...
function normalizeExclude(exclude: string | string[] | undefined): string {
  ...
  return Array.isArray(exclude) ? exclude.join(",") : String(exclude);
}
...
function thrashKey(
  ...
  return `${kind}|${cwd}|${pattern}|${path ?? ""}|${normalizeExclude(exclude)}`;
}

function checkThrash(key: string): string | null {
  const entry = recentSearches.get(key);
  if (!entry) return null;
  if (Date.now() - entry.ts > THRASH_WINDOW_MS) {
  ...
  entry.ts = Date.now();
  const tops =
    entry.lastTopPaths.length > 0
      ? entry.lastTopPaths.slice(0, 5).join(", ")
      : "(none)";
  return `Already searched this query. Top hits were: ${tops}. Read those files or refine pattern/path. Do not repeat the same grep.`;
}

function recordSearch(key: string, topPaths: string[]): void {
  recentSearches.set(key, {
    count: 1,
    lastTopPaths: topPaths.slice(0, 8),
    ts: Date.now(),
  });
  if (recentSearches.size > 100) {
    const first = recentSearches.keys().next().value;
    if (first) recentSearches.delete(first);
  }
}

/** Stock-style finish: truncateHead @ 50KB + optional Grep/Find details fields. */
function finishOutput(
  ...
  if (extra.matchLimitReached != null) {
    details.matchLimitReached = extra.matchLimitReached;
  }
  if (extra.resultLimitReached != null) {
    details.resultLimitReached = extra.resultLimitReached;
  }
...
function topPathsFromGrep(result: GrepResult, n = 8): string[] {
  const paths: string[] = [];
  const seen = new Set<string>();
  for (const match of result.items) {
    if (seen.has(match.relativePath)) continue;
    seen.add(match.relativePath);
    paths.push(match.relativePath);
...
export default function fffExtension(pi: ExtensionAPI) {
  ...
  // Mode resolution: flag > env > default (ours: override, not tools-and-ui)
  ...
  const grepSchema = Type.Object({
    ...
    path: Type.Optional(
      Type.String({
        description: "Dir, file, or glob under the workspace (e.g. src/, *.ts). Optional.",
      }),
    ),
    exclude: Type.Optional(
      Type.Union([Type.String(), Type.Array(Type.String())], {
        description: "Dirs/globs to skip (e.g. test/, node_modules/, *.min.js). Optional.",
      }),
    ),
  ...
  pi.registerTool({
    ...
    description: `Grep file contents (smart-case, git-aware, frecency-ranked). Default limit ${DEFAULT_GREP_LIMIT}. Output truncated to ${DEFAULT_GREP_LIMIT} matches or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first).`,
    promptSnippet: "Search file contents (ranked, git-aware)",
    // Lean: no always-on guidelines (stock pi find/grep also have none).
    promptGuidelines: [],
    ...
    async execute(_toolCallId, params, signal) {
      ...
      // Pagination cursors are not thrash — only identical fresh queries.
      if (!params.cursor) {
        const thrash = checkThrash(searchKey);
        if (thrash) {
          return {
            content: [{ type: "text", text: thrash }],
            details: undefined,
          };
        }
      }
      ...
      if (!params.cursor) {
        recordSearch(searchKey, topPathsFromGrep(result));
      }

      const matchLimitReached =
        Boolean(result.nextCursor) || result.items.length >= effectiveLimit;
      return finishOutput(
        output,
  ...
  const findSchema = Type.Object({
    pattern: Type.String({
      description:
        "Fuzzy path/filename terms or glob (e.g. store, **/schema.prisma). Multi-word = AND.",
    }),
    path: Type.Optional(
      Type.String({
        description: "Dir, file, or glob under the workspace (e.g. src/, *.ts). Optional.",
  ...
  pi.registerTool({
    ...
    description: `Find files by path/glob (fuzzy, frecency-ranked, git-aware). Default limit ${DEFAULT_FIND_LIMIT}. Output truncated to ${DEFAULT_FIND_LIMIT} results or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first).`,
    promptSnippet: "Find files by path/glob (ranked, git-aware)",
    promptGuidelines: [],
    ...
    async execute(_toolCallId, params, signal) {
      ...
      if (!resumed) {
        const thrash = checkThrash(findKey);
        if (thrash) {
          return {
            content: [{ type: "text", text: thrash.replace(/grep/g, "find") }],
            details: undefined,
          };
        }
      }
      ...
      if (!resumed) {
        const topPaths = result.items
          .map((item: { relativePath?: string; path?: string }) => item.relativePath || item.path || "")
          .filter(Boolean)
          .slice(0, 8);
        recordSearch(findKey, topPaths);
      }

      return finishOutput(
        output,
        hasMore ? { resultLimitReached: effectiveLimit } : {},
      );
    },
